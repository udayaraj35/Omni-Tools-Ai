...
import { Loader2, Scissors, File, X, Download, UploadCloud, CheckSquare, Square, Trash2 } from 'lucide-react';
...
export function RemovePages({ onTaskComplete }: { onTaskComplete: () => void }) {
        ...
        const loadPdfLib = async () => {
          try {
            ...
            if (typeof window !== 'undefined') {
                const workerSrc = `//cdnjs.cloudflare.com/ajax/libs/pdf.js/${'4.5.136'}/pdf.worker.mjs`;
    ...
    const onDrop = useCallback(async (acceptedFiles: File[]) => {
        ...
        if (newPreviews.length > 0) {
            toast({ title: 'Ready to go!', description: `Select pages to remove.` });
        }
    ...
    const handleRemove = async () => {
        if (!file || selectedPages.size === 0) {
            toast({ variant: 'destructive', title: 'No pages selected', description: 'Please select at least one page to remove.' });
            return;
        }

        setIsLoading(true);
        toast({ title: 'Removing pages...' });

        try {
            const existingPdfBytes = await file.arrayBuffer();
            const pdfDoc = await PDFDocument.load(existingPdfBytes);
            
            // Get sorted indices to remove, in descending order to avoid index shifting issues
            ...
            a.href = url;
            a.download = `removed-pages-omnitools.pdf`;
            ...
            toast({ variant: 'destructive', title: 'An error occurred', description: 'Could not remove pages from the PDF.' });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <Card>
            <CardContent className="p-6 space-y-4">
                ...
                {previews.length > 0 && (
                    <div className="space-y-2">
                         <div className="flex items-center justify-between">
                            <h4 className="font-semibold">Select pages to remove ({selectedPages.size} / {previews.length}):</h4>
                        ...
                        <div className="p-2 border rounded-md max-h-[40vh] overflow-y-auto grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                            {previews.map((page) => (
                                <div key={page.id} className="relative group cursor-pointer" onClick={() => togglePageSelection(page.pageNumber)}>
                                    ...
                                    <div className={`absolute inset-0 rounded-md transition-all ${selectedPages.has(page.pageNumber) ? 'ring-2 ring-destructive bg-destructive/20' : 'bg-black/0 group-hover:bg-black/20'}`}>
                                        {selectedPages.has(page.pageNumber) && (
                                            <div className="absolute top-2 left-2 bg-destructive text-destructive-foreground rounded-full p-1">
                ...
                <Button onClick={handleRemove} disabled={isLoading || selectedPages.size === 0} className="w-full h-12 text-base font-bold gradient-button-gold">
