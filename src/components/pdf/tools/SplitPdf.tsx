...
import { Loader2, Scissors, File, X, Download, UploadCloud, CheckSquare, Square } from 'lucide-react';
...
export function SplitPdf({ onTaskComplete }: { onTaskComplete: () => void }) {
    ...
    const onDrop = useCallback(async (acceptedFiles: File[]) => {
        ...
        if (newPreviews.length > 0) {
            toast({ title: 'Ready to split!', description: `Select pages to extract.` });
        }
    ...
    const handleExtract = async () => {
        ...
        try {
            ...
            a.href = url;
            a.download = `split-omnitools.pdf`;
    ...
    const handleSplitAll = async () => {
        if (!file || previews.length === 0) {
            toast({ variant: 'destructive', title: 'No PDF selected' });
            return;
        }

        setIsLoading(true);
        toast({ title: 'Splitting all pages...' });
        ...
        try {
            ...
            toast({ title: 'Success!', description: 'All pages have been split and downloaded as a ZIP file.' });
            onTaskComplete();

        } catch (error) {
            console.error('Error splitting all pages:', error);
            toast({ variant: 'destructive', title: 'An error occurred', description: 'Could not split the PDF.' });
        } finally {
            setIsLoading(false);
        }
    }
        ...
        <Card>
            <CardContent className="p-6 space-y-4">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                    ...
                    <Button onClick={handleSplitAll} disabled={isLoading || previews.length === 0} className="w-full h-12 text-base font-bold" variant="secondary">
